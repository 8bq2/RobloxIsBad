...
while true do
	...
	LS.C0 = LS.C0:lerp(LSC0 * CF.N(0, 0, 0 - 0.2 * (movement/8) * M.S(Sine/wsVal) * forwardvelocity) * 
    CF.A(
        M.R(0 + 45 * (movement/8) * M.S(Sine/wsVal) * forwardvelocity), 
        0, 
        M.R(-5 - 5 * M.C(Sine/wsVal))
    ), 
    Alpha
)
